...
fn digit_finder_strategies_benchmark(c: &mut Criterion) {
    ...
    c.bench_function("[Short Text] Iterator Strategy", |b| {
        b.iter(|| iterator.find_digit(black_box(short)))
    });
    c.bench_function("[Short Text] Two Pointers Strategy", |b| {
        b.iter(|| two_pointer.find_digit(black_box(short)))
    });

    let pattern = "threetwornnslsvxmsbcpvnbpfeveneightwokcn";
    let long = pattern.repeat(100) + "1" + &pattern.repeat(100);

    c.bench_function("[Long Message] Iterator Strategy", |b| {
        b.iter(|| iterator.find_digit(black_box(&long)))
    });
    c.bench_function("[Long Message] Two Pointers Strategy", |b| {
        b.iter(|| two_pointer.find_digit(black_box(&long)))
    });
}
